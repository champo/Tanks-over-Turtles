...
def assert_params_decorator(function):
    ''' Asserts that the function parameter receives two arguments:
    a tank and a direction '''
...
def logging_decorator(function):
    ''' Logs the call to the function decorated '''
    def new_function(*args, **kwds):
...
class Map(object):
    ...
    def get_size(self):
        ''' Returns the side of the rectangle (the name of the variable is
        called _dimension when stored as a object member). '''
        return self._dimension

    def get_shoot_range(self):
        ''' Returns the maximum range the shoots can reach '''
